...
// Selecao do Mux3 --> E´ so´ colocar: 0, 1, 2 ... 7  para selecionar os
// Registradores

// Selecao do Mux4 --> E´ so´ colocar: 0, 1, 2 ... 7  para selecionar os
// Registradores ou 8 para entrar o nr. 1
...
#define LOAD \
    48  // "110000"; -- LOAD Rx END  -- Rx <- M[END]  Format: < inst(6) | Rx(3)
        // | xxxxxxx >  + 16bit END
#define STORE \
    49  // "110001"; -- STORE END Rx -- M[END] <- Rx  Format: < inst(6) | Rx(3)
        // | xxxxxxx >  + 16bit END
#define LOADN \
    56  // "111000"; -- LOAD Rx Nr  (b0=0)   -- Rx <- Nr    ou   Load SP Nr
        // (b0=1)  -- SP <- Nr    Format: < inst(6) | Rx(3) | xxxxxxb0 >  +
        // 16bit Numero
#define LOADI \
    60  // "111100"; -- LOAD Rx Ry   -- Rx <- M[Ry]	Format: < inst(6) |
        // Rx(3) | Ry(3) | xxxx >
#define STOREI \
    61  // "111101"; -- STORE Rx Ry  -- M[Rx] <- Ry	Format: < inst(6) |
        // Rx(3) | Ry(3) | xxxx >
#define MOV \
    51  // "110011"; -- MOV Rx Ry    -- Rx <- Ry	  	Format: <
        // inst(6) | Rx(3) | Ry(3) | xxxx >

// I/O Instructions:
#define OUTCHAR \
    50  // "110010"; -- OUTCHAR Rx Ry -- Video[Ry] <- Char(Rx)
        // Format: < inst(6) | Rx(3) | Ry(3) | xxxx >
#define INCHAR \
    53  // "110101"; -- INCHAR Rx     -- Rx[7..0] <- KeyPressed	Rx[15..8] <- 0´s
        // Format: < inst(6) | Rx(3) | xxxxxxx >

// Aritmethic Instructions(All should begin wiht "10"):
#define ARITH 2
#define ADD \
    32  // "100000"; -- ADD Rx Ry Rz / ADDC Rx Ry Rz  	-- Rx <- Ry + Rz / Rx <-
        // Ry + Rz + C  -- b0=Carry	  	Format: < inst(6) | Rx(3) |
        // Ry(3) | Rz(3)| C >
#define SUB \
    33  // "100001"; -- SUB Rx Ry Rz / SUBC Rx Ry Rz  	-- Rx <- Ry - Rz / Rx <-
        // Ry - Rz + C  -- b0=Carry	  	Format: < inst(6) | Rx(3) |
        // Ry(3) | Rz(3)| C >
#define MULT \
    34  // "100010"; -- MUL Rx Ry Rz  / MUL Rx Ry Rz	-- Rx <- Ry * Rz / Rx <-
        // Ry * Rz + C  -- b0=Carry		Format: < inst(6) | Rx(3) |
        // Ry(3) | Rz(3)| C >
#define DIV \
    35  // "100011"; -- DIV Rx Ry Rz 			-- Rx <- Ry / Rz / Rx <-
        // Ry / Rz + C  -- b0=Carry		Format: < inst(6) | Rx(3) |
        // Ry(3) | Rz(3)| C >
#define INC \
    36  // "100100"; -- INC Rx / DEC Rx                 		-- Rx <-
        // Rx + 1 / Rx <- Rx - 1  -- b6= INC/DEC : 0/1	Format: < inst(6) |
        // Rx(3) | b6 | xxxxxx >
#define LMOD \
    37  // "100101"; -- MOD Rx Ry Rz   			-- Rx <- Ry MOD Rz
        // Format: < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
#define MID \
    38  // "100110"; -- MID Rx Ry Rz   			-- Rx <- (Ry + Rz) >> 1
        // Format: < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >

// Logic Instructions (All should begin wiht "01"):
#define LOGIC 1
#define LAND \
    18  // "010010"; -- AND Rx Ry Rz  	-- Rz <- Rx AND Ry	Format: <
        // inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
#define LOR \
    19  // "010011"; -- OR Rx Ry Rz   	-- Rz <- Rx OR Ry		Format:
        // < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
#define LXOR \
    20  // "010100"; -- XOR Rx Ry Rz  	-- Rz <- Rx XOR Ry	Format: <
        // inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
#define LNOT \
    21  // "010101"; -- NOT Rx Ry       	-- Rx <- NOT(Ry)
        // Format: < inst(6) | Rx(3) | Ry(3) | xxxx >
#define SHIFT \
    16  // "010000"; -- SHIFTL0 Rx,n / SHIFTL1 Rx,n / SHIFTR0 Rx,n / SHIFTR1
        // Rx,n / ROTL Rx,n / ROTR Rx,n
//           -- Format: < inst(6) | Rx(3) |  b6 b5 b4 | nnnn >
#define CMP \
    22  // "010110"; -- CMP Rx Ry  		-- Compare Rx and Ry and set FR
        // :  FL = <...|over|carry|zero|equal|lesser|greater>	  Format: <
        // inst(6) | Rx(3) | Ry(3) | xxxx >

// FLOW CONTROL Instructions:
#define JMP \
    2  // "000010"; -- JMP END    -- PC <- 16bit END
       // : b9-b6 = COND		Format: < inst(6) | COND(4) | xxxxxx >
       // + 16bit END
#define CALL \
    3  // "000011"; -- CALL END   -- M[SP] <- PC | SP-- | PC <- 16bit END   :
       // b9-b6 = COND	  	Format: < inst(6) | COND(4) | xxxxxx >   + 16bit
       // END
#define RTS \
    4  // "000100"; -- RTS        -- SP++ | PC <- M[SP] | b6=Rx/FR: 1/0
       // Format: < inst(6) | xxxxxxxxxx >
#define PUSH \
    5  // "000101"; -- PUSH Rx / PUSH FR  -- M[SP] <- Rx / M[SP] <- FR | SP--
       // : b6=Rx/FR: 0/1		Format: < inst(6) | Rx(3) | b6 | xxxxxx
       // >
#define POP \
    6  // "000110"; -- POP Rx  / POP FR   -- SP++ | Rx <- M[SP]  / FR <- M[SP]
       // : b6=Rx/FR: 0/1		Format: < inst(6) | Rx(3) | b6 | xxxxxx
       // >

// Control Instructions:
#define NOP \
    0  // "000000"; -- NOP            -- Do Nothing
       // Format: < inst(6) | xxxxxxxxxx >
#define HALT \
    15  // "001111"; -- HALT           -- Stop Here
        // Format: < inst(6) | xxxxxxxxxx >
#define SETC \
    8  // "001000"; -- CLEARC / SETC  -- Set/Clear Carry: b9 = 1-set; 0-clear
       // Format: < inst(6) | b9 | xxxxxxxxx >
#define BREAKP \
    14  // "001110"; -- BREAKP         -- Break Point: Pausa execussao
        // Format: < inst(6) | xxxxxxxxxx >
...
unsigned int MEMORY[TAMANHO_MEMORIA];  // Vetor que representa a Memoria de
                                       // programa e de dados do Processador
...
int FR[16] = {
    0};  // Flag Register:
         // <...|Negativo|StackUnderflow|StackOverflow|DivByZero|ArithmeticOverflow|carry|zero|equal|lesser|greater>

int main() {
    int i = 0;
    int key = 0;  // Le Teclado
    int PC = 0, IR = 0, SP = 0, MAR = 0, rx = 0, ry = 0, rz = 0, COND = 0,
        RW = 0, DATA_OUT = 0;
    int LoadPC = 0, IncPC = 0, LoadIR = 0, LoadSP = 0, IncSP = 0, DecSP = 0,
        LoadMAR = 0, LoadFR = 0;
    int M1 = 0, M2 = 0, M3 = 0, M4 = 0, M5 = 0, M6 = 0;
    int selM1 = 0, selM2 = 0, selM3 = 0, selM4 = 0, selM5 = 0, selM6 = 0;
    int LoadReg[8] = {0};
    int carry = 0;  // Flag do IR que indica se a ULA vai fazer operação com
                    // carry ou não
    int opcode = 0;
    int temp = 0;
...
loop:
    ...
    switch (state) {
        case STATE_RESET:
            ...
            PC = 0;  // inicializa na linha Zero da memoria -> Programa tem que
                     // comecar na linha Zero !!
            IR = 0;
            MAR = 0;
        ...
        case STATE_DECODE:
            ...
            switch (opcode) {
                ...
                case INC:
                    ...
                    selM3 = rx;
                    selM4 =
                        8;  // 8 para selecionar o nr. 1 como entrada do MUX4
                ...
                case JMP:
                    ...
                    if ((COND == 0)                     // NO COND
                        ...
                        || ((FR[2] == 1 || FR[0] == 1) &&
                            (COND == 9))                // GREATER EQUAL
                        || (FR[1] == 1 && (COND == 8))  // LESSER
                        || ((FR[2] == 1 || FR[1] == 1) &&
                            (COND == 10))                 // LESSER EQUAL
                        || (FR[2] == 1 && (COND == 1))    // EQUAL
    ...
    if (selM4 == 8)
        M4 = 1;  // Seleciona 1 para fazer INC e DEC
    else
        M4 = reg[selM4];
    // Selecao do Mux1
    if (selM1 == sPC)
    ...
    if (M1 > (TAMANHO_MEMORIA)) {
        M1 = 0;
        printf(
            "  \n\nUltrapassou limite da memoria, coloque um jmp no fim do "
            "código\n");
        exit(1);
    }
    ...
    // Selecao do Mux2
    if (selM2 == sULA)
        M2 = resultadoUla.result;
    else if (selM2 == sDATA_OUT)
        M2 = DATA_OUT;
    else if (selM2 == sM4)
        M2 = M4;
    ...
    // Selecao do Mux6
    if (selM6 == sULA)
        M6 = resultadoUla.auxFR;  // TODO: Talvez o auxFR deva ser o valor do FR
                                  // //**Sempre recebe flags da ULA
    else if (selM6 == sDATA_OUT)
        M6 = DATA_OUT;  //** A menos que seja POP FR, quando recebe da Memoria
...
void le_arquivo(void) {
    ...
    int i, j;
    int processando =
        0;  // Flag para varreo o arquivo CPURAM.mif e tirar o cabecalho

    if ((stream = fopen("cpuram.mif", "r")) ==
        NULL)  // Abre o arquivo para leitura
    {
    ...
    while (fscanf(stream, "%s", linha) !=
           EOF)  // Le linha por linha ate' o final do arquivo: eof = end of
                 // file !!
    {
        char letra[2] = "00";
        ...
        if (!processando) {
            i = 0;
            do  // Busca por sequencias de caracteres para catar inicio do
                // codigo
            {
                letra[0] = letra[1];
                letra[1] = linha[i];
                if ((letra[0] == '0') &&
                    (letra[1] ==
                     ':'))  // Cata primeira linha de codigo omecando com "0: "
                {
...
int processa_linha(char* linha) {
    ...
    for (j = 0; j < 16; j++)  // le a palavra toda
    {
        ...
        valor += linha[i + j + 1] -
                 '0';  // converte char pra numero 0 ou 1 o bit 15-k da palavra
    }
...
// ULA
ResultadoUla ULA(unsigned int x, unsigned int y, unsigned int OP, int carry) {
    unsigned int auxFRbits[16] = {
        0};  // TODO ficar 0 quando der reset? Nao entendi o auxFR
    unsigned int result = 0;

    // printf("OP:%d - arith:%d add:%d\n", OP, pega_pedaco(OP, 5, 4),
    // pega_pedaco(OP, 3, 0));
            ...
            if (OP == CMP) {
                ...
                switch (OP) {
                    ...
                    case LNOT:
                        ...
                        // & MAX_VAL -> para garantir que vai ficar menor igual
                        // que 65535
                        result = ~x & MAX_VAL;
                        break;
                    default:
    ...
    for (int i = 16; i--;) auxFR = auxFR + (int)(auxFRbits[i] * (pow(2.0, i)));
